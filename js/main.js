...
var player, shadow, monsters = [], asideMiles = [], asideCheers = [], asideCheers2 = [];
var winWidth, winHeight, guideStatus = 0, isGuide = false;
var startTouchPoint, touchCacheX = 0.12, touchCacheY = 0.2;
var startTime, pauseTime, gmfCounts = 0, stepLength = 1800;
// 初始化页面
$(function() {
	loadPlayerCnt();
	initAudio();
...
var showGuide = function(index) {
	...
	if (index === 1) {
		...
		var container = $('#guide').find('.container');
		container.removeClass('guide_bg_2').addClass('guide_bg_1').find('article').html(
		    'GIME ME FIVE！<br/>给市运会加油！<br/>');
		$('#guide').show();
	}
...
var nextMonster = false, monIndex = 0, nextMonTime = null, currTime = null, noMoreMonster = false;
...
var renderMonster = function() {
	...
	if (!nextMonster && !noMoreMonster && !isGuide) {
		var pathWidth = winWidth / 3, type = guideSortIndex[monIndex], w = pathWidth / 3 * 2, h = pathWidth / 3 * 2 * 1.4;
		if (type === 2) {
...
var nextAsideMile = false, mileIndex = 0, nextMileTime = 0;
// 顺序加载数字
...
var nextAsideCheer = false, cheerIndex = 1, nextCheerTime = 0;
// 左边啦啦队
var renderAsideCheer = function() {
	...
	if (!nextAsideCheer) {
		...
		var temp = new AsideCheer(getRoundVal(0, 1) === 0 ? (getRoundVal(0, 1) === 0 ? 1 : 3)
		    : (getRoundVal(0, 1) === 0 ? 2 : 4), 1, 90, 160, cheerIndex);
...
var nextAsideCheer2 = false, cheerIndex2 = 1, nextCheerTime2 = 0;
// 右边啦啦队
var renderAsideCheer2 = function() {
	...
	if (!nextAsideCheer2) {
		...
		var temp = new AsideCheer(getRoundVal(0, 1) === 0 ? (getRoundVal(0, 1) === 0 ? 3 : 4)
		    : (getRoundVal(0, 1) === 0 ? 1 : 2), 2, 90, 160, cheerIndex2);
...
var formatMilli = function(milli) {
	var s = parseInt(milli / 1000), m = milli % 1000;
	m = parseInt(m / 100);
